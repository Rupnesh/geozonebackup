                    ...
                    if method is not None and body is not None:
                        try:
                            ...
                            if body.find("[SDCARDSTATUS]") >=0:
                                ...
                                print(arr)
                                resp = Response(json.dumps(arr), status=200, mimetype=JSON_MIME_TYPE)
                                resp.headers['Access-Control-Allow-Origin'] = '*'
                                resp.headers['Access-Control-Allow-Credentials'] = True
                                resp.headers['Access-Control-Allow-Methods']= '*'
                                resp.headers['Access-Control-Allow-Headers'] = '*'
                    ...
                    try:
                        if message_tuple is not None:  # None returned after inactivity timeout
                            ...
                            if method is not None and body is not None:
                                try:
                                    body=body.decode()
                                    if body.find("[SDCARDSTATUS]\r\n") >=0:
                                        receivedSpaceData = processOnLogging(body)
                                        if type(receivedSpaceData) == list:
                                            if FileName in receivedSpaceData:
            ...
            for fname in files:
                ...
                try:
                    ...
                    if fname == filename1:
                        ...
                        rv = send_file(fileNamePath['path'],mimetype="text",as_attachment=True)
