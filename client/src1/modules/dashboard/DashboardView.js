...
class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    socket && socket.emit('subscribewifi');
    ...
    socket.on('data-wifi', this.handleWIFI);
    socket.on('data-wifi-current', this.handleWIFICurrent);
  ...
  handleIMU1Data(data) {
    ...
    //var data =JSON.parse(data); ///for testing
  ...
  handleGPSJSONData(data) {
    const parsedData = data;  //for testing
  ...
  updateSatellites(data) {
    ...
    if (this.satelliteData.satellites.length > 0) {
      ...
      // this.state.isSkyPlotDataAvailable = true  //for testing
  ...
  plotCalculations = (onResize) => {
    ...
    for (let i = 0; i < svx.length; i++) {
      ...
      newImage.onload = () => {
        ...
        //console.log('testMy', newImage.width * imageSizeOffset+'height'+newImage.height * imageSizeOffset)
      };
  ...
  spiritLevelCalculations(value) {
    ...
    if ((parseInt(scaleCH(CFangleX)) >= 1 && parseInt(scaleCH(CFangleX)) <= 73) && (parseInt(scaleCV(CFangleY)) >= 1 && parseInt(scaleCV(CFangleY)) <= 62)) {
      ...
      // console.log("POSITION",newcBposition);
      // console.log("POS X",CFangleX);
      // console.log("POS Y",CFangleY);
    }
    //else {
    //newcBposition = [73, 52];
    //}
    ...
    if (JSON.stringify(newData) !== JSON.stringify(this.spiritLevelData)) {
      ...
      cBubble.onload = () => {
        if (newData['cBposition'][0] == 41 && newData['cBposition'][1] == 43) {
          cx.drawImage(cBubble, newData['cBposition'][0], newData['cBposition'][1], bubbleWidth, bubbleWidth);
        }
        else {
          cx.drawImage(cBubble1, newData['cBposition'][0], newData['cBposition'][1], bubbleWidth, bubbleWidth);
        }

      };
      cBubble1.src = "img/spirit-level/Vectorillustration_design_4_center_bubble.png";
      cBubble.src = "img/spirit-level/Vectorillustration_design_4_center_bubble_dark.png"; // tesing
    }
  };
