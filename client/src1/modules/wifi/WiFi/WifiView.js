...
class WifiView extends Component {
  ...
  getStatusOfWiFi = () => {
    AxiosPromise.get(api.wifiStatus)
      .then(response => {
        if (response.data) {
          this.setState({
            ConnectedStatus: response.data.status, statusMSG1: response.data.message1,
  ...
  loginToWIFI = () => {
    ...
    AxiosPromise.post(api.LOGIN_WITH_WIFI, data)
      .then(response => {
        if (response.data) {
          this.setState({
            ConnectedStatus: response.data.status,
  ...
  render() {
    ...
    const { nf_email, nf_password } = this.state;
          ...
          <div className="col-sm-6">
            <div className="card">
              ...
              <div className="card-footer">
                <button onClick={() => this.loginToWIFI()} type="submit"
                  // disabled={!this.state.WIFI} 
                  disabled={!enabled}
                  style={cursorStyle} className="btn btn-sm btn-success">
                  <i className="fa fa-dot-circle-o"></i> Connect to WiFi
                </button>
              </div>
            </div>
          </div>
