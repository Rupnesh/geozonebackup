...
def sendToQueue(data):
    #url = os.environ.get('CLOUDAMQP_URL', 'amqp://oynylrww:GgJXP1k3PYaAZ_8NXPmNbf8OZRyTnu3k@donkey.rmq.cloudamqp.com/oynylrww')
    #params = pika.URLParameters(url)
    #params.socket_timeout = 5
    #connection = pika.BlockingConnection(params)
    connection = pika.BlockingConnection(pika.ConnectionParameters('localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='IMU_1')
    channel.basic_publish(exchange="",routing_key='IMU_1',body=data)
    connection.close()
...
while True:
	...
	#build json
	data = '{"loopTime": %5.2f, "ACCX Angle": %5.2f, "ACCY Angle": %5.2f, "GRYX Angle": %5.2f, "GYRY Angle": %5.2f, "GYRZ Angle": %5.2f, "cfangleX": %5.2f, "cfangleY": %5.2f, "cfangleZ": %5.2f, "HEADING": %5.2f, "tiltCompensatedHeading": %5.2f, "kalmanX": %5.2f, "kalmanY": %5.2f}' % (LP, AccXangle, AccYangle, gyroXangle, gyroYangle, gyroZangle, CFangleX,CFangleY,CFangleZ, heading, tiltCompensatedHeading, kalmanX, kalmanY)
	sendToQueue(data)
